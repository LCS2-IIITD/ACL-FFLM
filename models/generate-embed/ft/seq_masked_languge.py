...
# parser.add_argument("-m","--model", type = str, help = "display the square of a given number")
...
model_names = ["bert-base-cased","roberta-base","xlnet-base-cased","gpt2"]
numClasses = [75]
train_possibilities = [800]
test_possibilities = [200]
#---------------------------------------------------
...
nf_train_samples = train_possibilities[0] 
nf_test_samples = test_possibilities[0]
num_classes = numClasses[0]

loss_type = 'cross_entropy'
# loss_type = 'multilabel'
one_hot = False
if loss_type == 'multilabel': one_hot = True

#------------------------------------------PATHS----------------------------------------------------------
path_str = 'balanced_' + str(num_classes) + '_' + str(nf_train_samples) + '_' + str(nf_test_samples)

working_folder = "/home/nirav17072/BTP/"
store_folder = working_folder + 'pipeline/experiment_3_gltr/finetuned_classifier_1/store/'
...
finetuned_embeddings_folder = store_folder + 'finetuned_embedding/best/' + model_name + '/' + loss_type + '/'
best_model_test_embeddings_path = finetuned_embeddings_folder + path_str + '.pkl'

best_predictions_folder = store_folder + 'test_predictions/best/' + model_name + '/' + loss_type + '/'
best_predictions_folder = best_predictions_folder + path_str + '/'
...
print('An example of the tokenizer  in use .....')
...
for sent in sentences:
	...
	if model_name == 'gpt2' or model_name == 'openai-gpt': sent += ' [CLS]'
...
for sent in sentences:
    sent = str(sent) # Converting Sentence into String
    if lowercase_sentence == True : sent = sent.lower() # Converting sentence into lowercase
...
def split_dataset(dataset, nfclasses = 30, num_splits = 5, one_hot = False):
    per_class_count, X, Y = per_class_distribution(dataset, nfclasses = nfclasses, one_hot = one_hot)
    print(per_class_count)
    ...
    for train_index, test_index in skf.split(X, Y):
        ...
        for i in train_index:
            if one_hot == True:
                y = np.argmax(np.array(Y[i]).reshape(-1,1),axis = 0)
            else:
                y = int(Y[i])
            per_class_count[int(y)] += 1 
                
        
#         print('--- Train Fold Distribution---', k)
#         print(per_class_count)
        k += 1
...
def custom_split(dataset, num_splits = 5, nf_train_samples = 800, nfclasses = 30, fold_no = 1, one_hot = False):
    
    # Dividing into train set and
    indices = [index for index in range(len(dataset))]
    
    train_set = Subset(dataset, indices[:nf_train_samples * nfclasses])
    test_set = Subset(dataset, indices[nf_train_samples * nfclasses:])
    
    print('Number of  Samples in train set = ',len(train_set))
    
    training_indices, _ = split_dataset(train_set, nfclasses, num_splits, one_hot = one_hot)
    
    print(training_indices)
    
    train_indices, _ = split_dataset(train_set, nfclasses, num_splits)
    fold_train_set = Subset(dataset, training_indices[fold_no - 1])

    print('Number of  Samples in Fold1 Training Set =',len(fold_train_set))
    print('Number of  Samples in Testing Set =',len(test_set))
    
    return fold_train_set, train_set, test_set, training_indices
...
fold_train_set, train_set, test_set, training_indices = custom_split(dataset, num_splits = 5, nf_train_samples = nf_train_samples, nfclasses = num_classes, one_hot = one_hot )

train_dataset = fold_train_set
test_dataset = test_set

print('{:>5,} training samples'.format(len(train_dataset)))
print('{:>5,} Train samples'.format(len(test_dataset)))

for i in range(5):
    x = tokenizer.convert_ids_to_tokens(test_dataset[i][0])
...
# Number of training epochs. The BERT authors recommend between 2 and 4. I ran for 5
epochs = 5
    ...
    for batch_embeddings in hidden_states:
        ...
        for embed in batch_embed_numpy: 
            if model_name[:4] == 'bert' or model_name[:4] == 'robe':
                ...
                final_embeddings.append(embed[0])
...
def evaluate(model, dataloader, model_name):
    ...
    for batch in dataloader:
        ...
        if batch_no % 1000 == 0 :
            print(all_label_ids.shape, all_logits.shape, len(all_hidden_states),all_hidden_states[0].shape)
...
for epoch_i in range(0, epochs):
    ...
    if test_accuracy > best_accuracy:
        ...
        dumpJsonFile(test_results_json, best_results_folder + 'epoch_' + str(epoch_i) + '_results_train.json')
